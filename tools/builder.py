...
def dataset_builder(args, config):
    # No need to extract or merge `_base_` – `config` is already complete
    default_args = config.others

    dataset = build_dataset_from_cfg(config, default_args=default_args)

    sampler = None
    dataloader = torch.utils.data.DataLoader(
        dataset,
        batch_size=default_args.bs,
        shuffle=(default_args.subset == 'train'),
